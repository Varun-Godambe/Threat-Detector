...
// It keeps the DOM manipulation logic separate from the main application flow.
...
export function displayAiLogReport(reportData, resultsContainer) {
    ...
    if (reportData.analysisResult === "No security issues or notable events found.") {
        ...
        const { summaryStats, findings } = reportData.analysisResult;
        ...
        dashboardContainer.querySelectorAll('.ai-tab-btn').forEach(button => {
            button.addEventListener('click', () => {
                dashboardContainer.querySelectorAll('.ai-tab-btn').forEach(btn => {
    ...
    header.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div class="stat-card bg-muted p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-muted-foreground">Total Findings</p>
                    <p class="text-3xl font-bold text-foreground">${totalFindings}</p>
                </div>
                <div class="stat-card bg-red-100 dark:bg-red-900/50 p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-red-600 dark:text-red-400">Critical</p>
                    <p class="text-3xl font-bold text-red-700 dark:text-red-300">${stats.critical || 0}</p>
                </div>
                <div class="stat-card bg-orange-100 dark:bg-orange-900/50 p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-orange-600 dark:text-orange-400">High</p>
                    <p class="text-3xl font-bold text-orange-700 dark:text-orange-300">${stats.high || 0}</p>
                </div>
                <div class="stat-card bg-yellow-100 dark:bg-yellow-900/50 p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-yellow-600 dark:text-yellow-400">Medium</p>
                    <p class="text-3xl font-bold text-yellow-700 dark:text-yellow-300">${stats.medium || 0}</p>
                </div>
                <div class="stat-card bg-blue-100 dark:bg-blue-900/50 p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-blue-600 dark:text-blue-400">Low</p>
                    <p class="text-3xl font-bold text-blue-700 dark:text-blue-300">${stats.low || 0}</p>
                </div>
                <div class="stat-card bg-gray-100 dark:bg-gray-700/50 p-4 rounded-lg text-center">
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Info</p>
                    <p class="text-3xl font-bold text-gray-700 dark:text-gray-300">${stats.informational || 0}</p>
                </div>
            </div>
    ...
    donutChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    position: 'right',
                    labels: { color: isDark ? '#e5e7eb' : '#1f2937', boxWidth: 12, padding: 15 }
                }
            }
        }
    });
...
function createFindingCard({ category, severity, timestamp, title, details, action }) {
    ...
    card.innerHTML = `
        <div class="flex flex-col sm:flex-row justify-between sm:items-start gap-2">
            <h4 class="font-semibold text-foreground flex-1">${title}</h4>
            <div class="flex items-center gap-2 flex-shrink-0">
                <span class="text-xs font-mono text-muted-foreground">${timestamp}</span>
        ...
        <div class="mt-3 pt-3 border-t border-border">
            <p class="text-sm"><strong class="font-semibold text-foreground">Recommended Action:</strong> <span class="text-muted-foreground">${action}</span></p>
        </div>
    `;
    return card;
}
...
export function displayVirusTotalUrlReport(report, resultsContainer) {
    const stats = report.attributes.stats;
    const maliciousCount = stats.malicious;
    const suspiciousCount = stats.suspicious;
    const totalVotes = stats.harmless + maliciousCount + suspiciousCount + stats.undetected;
